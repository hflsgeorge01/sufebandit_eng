...
distributions <- round(
  cbind(runif(n = 1e5, min = 15, max = 35),
        runif(n = 1e5, min = 10, max = 30),
        runif(n = 1e5, min = 5, max = 25)), 0)

goal<-20
trials.n <- 20                          # Trials per game
...
GameData <- reactiveValues(game = c(),
  ...
  PageLayouts <- reactive({
    ...
    if (CurrentValues$page == "instructions") {
      ...
      return(
        list(
          ...
          p("Choose an investment option for each round."),
          p("..."),
          p("..."),
    ...
    if (CurrentValues$page == "game") {
      ...
      return(
        list(
          # Main Display
          fixedRow(
            column(12, h2("Choose a box to invest")),
    ...
    if (CurrentValues$page == "gameend") {
      ...
      return(list(h3("Go to survey"),
                  actionButton(inputId = "gt_goodbye", 
                               label = "Save and view your result")))
    }
    ...
    if (CurrentValues$page == "goodbye") {
      ...
      return(list(
        h3("Here is your earnings graph"),
        plotOutput('EarningsPlot'),
        h2("Your final score is: ", span(CurrentValues$points.cum, style="color:red"),". Take a screenshot"),
